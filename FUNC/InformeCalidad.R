...
informeCalidad <- function(listadoMuestras, umbral_calidad = 20){
  ...
  archivoRmd <- generarRmd(directorio_salida, timestamp, umbral_calidad)
  
  rmarkdown::render(archivoRmd, quiet = TRUE, output_file = "informe.html")
  browseURL(file.path(directorio_salida, "informe.html"))
  
  cat("Informe disponible en:", directorio_salida, "\n")
...
extraerCalidadFastq <- function(listadoMuestras, subsample = 100000) {
  ...
  contenido_gc <- data.frame()
  
  for (i in seq_along(listadoMuestras)) {
    
    archivo <- listadoMuestras[i]
    nombre_muestra <- gsub("\\.fastq(\\.gz)?$", "", basename(archivo))
    cat(sprintf(" [%d/%d] Procesando: %s\n", i, length(listadoMuestras), nombre_muestra))
    
    
    fq_stream <- FastqStreamer(archivo, n = subsample)
    on.exit(close(fq_stream), add = TRUE)
    
    fq <- tryCatch(
      yield(fq_stream),
      error = function(e) {
        warning(sprintf("Error leyendo %s: %s", archivo, e$message))
        return(NULL)
      }
    )
    ...
    if (is.null(fq) || length(fq) == 0) next
    
    # 1. CALIDAD POR CICLO
    matrix_qa <- as(quality(fq), "matrix")
    calidad_media_ciclo <- colMeans(matrix_qa, na.rm = TRUE)
    
    calidad_ciclo <- rbind(calidad_ciclo, data.frame(
      muestra = nombre_muestra,
      ciclo = seq_along(calidad_media_ciclo),
      calidad_media = calidad_media_ciclo,
    ...
    # Contenido GC
    gc <- letterFrequency(sread(fq), "GC", as.prob = TRUE)
    
    estadisticas_generales <- rbind(estadisticas_generales, data.frame(
      muestra = nombre_muestra,
      total_reads = length(fq),
      calidad_media = mean(matrix_qa, na.rm = TRUE),
      longitud_media = mean(width(fq)),
      gc_content = mean(gc) * 100,
      stringsAsFactors = FALSE
    ))
    
    # 3. DISTRIBUCIÓN GC (para gráfico)
    contenido_gc <- rbind(contenido_gc, data.frame(
      muestra = nombre_muestra,
      porcentaje_gc = as.numeric(gc) * 100,
      stringsAsFactors = FALSE
    ))
  }
  ...
  return(list(
    ...
    gc_dist = contenido_gc,
    n_muestras = length(listadoMuestras)))
}


generarRmd <- function(directorio_salida, timestamp, umbral_calidad) {
  
  archivoRmd <- file.path(directorio_salida, "informe.Rmd")
  ...
  contenido <- c(
    ...
    '  geom_line(data = filter(calidad_ciclo, !problematica),',
    ...
    '                       paste(length(muestras_problematicas), "muestra(s) con calidad < Q", umbral_calidad),',
    ...
    '} else {',
    '  cat("**Problemas de calidad detectados en", porc_problematicas, "% de las muestras.**\\n\\n")',
    '}',
    '```',
    '',
    '---',
    '',
    paste0('*Informe generado el ', timestamp, '- Umbral de calidad: Q', umbral_calidad, '*')
  )
