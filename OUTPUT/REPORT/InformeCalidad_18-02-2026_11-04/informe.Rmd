...
muestras_problematicas <- estadisticas %>%
  filter(calidad_media < umbral_calidad) %>%
  pull(muestra)
...
if (nrow(muestras_bajas) > 0) {
  cat("\n **Atención:** Se detectaron", nrow(muestras_bajas),
      "muestra(s) con calidad media inferior a Q", umbral_calidad, ":\n\n")
  print(kable(
    muestras_bajas %>%
  ...
  cat("\n**Excelente:** Todas las muestras tienen calidad media ≥ Q", umbral_calidad, "\n")
}
```
...
ggplot(estadisticas, aes(x = calidad_media)) +
  ...
  geom_vline(xintercept = umbral_calidad, linetype = "dotted", color = "orange", linewidth = 1.2) +
...
n_problematicas <- sum(estadisticas$calidad_media < umbral_calidad, na.rm = TRUE)
...
*Informe generado el 18-02-2026_11-04- Umbral de calidad: Q20*
