...
Los datos utilizados en este trabajo se obtuvieron de la base de datos del European Nucleotide Archive (ENA), específicamente del proyecto PRJEB34168. En total, se descargaron 65 muestras: 39 correcpondientes a personas sanas y 26 a pacientes con esclerosis múltiple. Las muestras se descargaron en formato .fastq, ya que es el único formato disponible en el proyecto que permite diferenciar entre muestras de personas con esclerosis múltiple y personas sanas.
...
Por otro lado, probé a utilizar distintas funciones el paquete ShortRead:
-   nFilter()
-   trimTaiklw()
-   width()
...
En conclusión, utilizar filterAndTrim() de DADA2 resulta más recomendable, ya que es la función específicamente diseñada para el procesamiento de lecturas de secuenciación. Por su parte, el segundo enfoque (filtrado manual), aunque más flexible, requiere una mayor cantidad de código, lo que también incrementa la probabilidad de cometer errores.